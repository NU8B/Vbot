...
def create_dataset():
    data = []
    os.makedirs("data_train/raw_audio", exist_ok=True)
    os.makedirs("data_train/processed", exist_ok=True)

    for filename in os.listdir("data_train/raw_audio"):
        if filename.endswith((".wav", ".mp3", ".m4a")):
            try:
                # Process audio
                input_path = os.path.join("data_train/raw_audio", filename)
                base_filename = Path(filename).stem
                output_path = f"data_train/processed/{base_filename}.wav"
