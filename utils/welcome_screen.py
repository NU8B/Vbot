...
from .user_preferences import get_user_preferences, record_avatar_selection, get_user_profile_for_recommendation
...
class AvatarRecommender:
    ...
    def recommend_avatar(self, preferences: Dict) -> str:
        ...
        for avatar_name, profile in self.avatar_profiles.items():
            ...
            # Personality tags matching (weight: 1 per match)
            avatar_tags = profile["tags"]
            for pref in preferred_personality:
                if pref.lower() in [tag.lower() for tag in avatar_tags]:
                    score += 1
        ...
        # Return avatar with highest score, default to Amelia if tie
        if scores:
            recommended = max(scores.items(), key=lambda x: x[1])
            return recommended[0] if recommended[1] > 0 else "Amelia"
...
class RecommendationDialog:
    ...
    def __init__(self, parent, recommender: AvatarRecommender, on_recommendation: Callable[[str], None]):
        self.parent = parent
    ...
    def _create_dialog_content(self):
        ...
        personality_options = [
            "Energetic", "Calm", "Playful", "Intellectual",
            "Mysterious", "Friendly", "Sophisticated", "Reliable"
        ]
    ...
    def _get_recommendation(self):
        ...
        dialog_preferences = {
            "gender": self.gender_var.get() if self.gender_var.get() != "no_preference" else "",
    ...
    def _show_recommendation_result(self, recommended_avatar: str, preferences: Dict):
        ...
        info_frame = tk.Frame(result_frame, bg="#2b2b3b", relief=tk.RAISED, borderwidth=1)
