...
from ai_avatar import AnimatedAvatar
import os
import requests
import re
...
class AIStyleTTS2:
    def __init__(self, root):
        # Initialize GUI
        self.root = root
        self.root.title("AI Assistant (Mistral + StyleTTS2)")
...
Assistant:"""
    ...
    def clean_text_for_tts(self, text):
        ...
        # Convert numbers to words using num2words with proper formatting
        def replace_num(match):
            try:
                num = int(match.group())
                # Use 'ordinal' for dates, 'cardinal' for other numbers
                if len(str(num)) == 4:  # Likely a year
        ...
        # Keep only specific punctuation marks and replace others with spaces
        cleaned_text = re.sub(r'[^\w\s,.!?\'"-]', " ", converted_text)
